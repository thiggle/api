...
export type TypedCompletionRequest = {
  prompt: string;
  schema: string;
  type_name: string;
  max_tokens?: number;
...
export type TypedCompletionResponse = {
  id: string;
  object: string;
  created: number;
  choices: CompletionChoice[];
...
class Thiggle {
  ...
  async regexCompletion(
    request: RegexCompletionRequest
  ): Promise<RegexCompletionResponse> {
    return this.makeRequest<RegexCompletionResponse>(
      "/v1/completion/regex",
      request
    );
  }

  async contextFreeCompletion(
    request: ContextFreeCompletionRequest
  ): Promise<ContextFreeCompletionResponse> {
    return this.makeRequest<ContextFreeCompletionResponse>(
      "/v1/completion/cfg",
      request
    );
  }

  async typedCompletion(
    request: TypedCompletionRequest
  ): Promise<TypedCompletionResponse> {
    return this.makeRequest<TypedCompletionResponse>(
      "/v1/completion/typed",
      request
    );
  }
